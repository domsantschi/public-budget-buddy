...
const ExportPreview = ({ selectedCanton, selectedAccount, aiSuggestion }: ExportPreviewProps) => {
  ...
  return (
    <Card className="w-full h-full flex flex-col animate-fade-in bg-white/50 backdrop-blur-sm">
      <div className="p-4 border-b flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <Coins className="h-5 w-5 text-muted-foreground" />
          <h3 className="font-semibold">Budget Comparison</h3>
        </div>
          ...
          <section className="space-y-4">
            ...
            <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
              ...
              <div className="text-sm text-blue-600 mt-1">
                {userEstimate ? (
                  difference > 0 
                    ? "Your estimate is higher than AI suggestion. Consider reviewing for potential savings."
                    : difference < 0
                      ? "Your estimate is lower than AI suggestion. This may lead to budget shortfalls."
                      : "Your estimate matches the AI suggestion exactly."
                ) : (
                  "Enter your budget estimate to see the comparison."
                )}
              </div>
            </div>
          </section>
